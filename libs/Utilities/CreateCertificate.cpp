...
PCCERT_CONTEXT CreateCertificate(
   ...
   HCRYPTKEY hKey = NULL;
   const WCHAR * KeyContainerName = L"SSLTestKeyContainer";
   ...
   try {
      ...
      if (!CryptAcquireContextW(
             &hCryptProv, KeyContainerName, NULL, PROV_RSA_FULL, KeyFlags)) {
         ...
         if (!CryptAcquireContextW(
                ...
                NULL,
                PROV_RSA_FULL,
                KeyFlags | CRYPT_NEWKEYSET)) {
            int err = GetLastError();

            if (err == NTE_EXISTS)
               DebugMsg(
                  "**** CryptAcquireContext failed with 'already exists', are you running as administrator");
            else
               DebugMsg(
                  "**** Error 0x%x returned by CryptAcquireContext", err);
            // Error
   ...
   // CREATE SELF-SIGNED CERTIFICATE AND ADD IT TO PERSONAL STORE IN MACHINE
   // PROFILE
   ...
   try {
      ...
      KeyProvInfo.pwszContainerName =
         (LPWSTR)KeyContainerName; // The key we made earlier
      ...
      // Specify the allowed usage of the certificate (client or server
      // authentication)
      DebugMsg(("CertAddEnhancedKeyUsageIdentifier"));
      LPCSTR szOID =
         MachineCert ? szOID_PKIX_KP_SERVER_AUTH : szOID_PKIX_KP_CLIENT_AUTH;
      ...
      cdblob.cbData =
         (DWORD)(wcslen((LPWSTR)cdblob.pbData) + 1) * sizeof(WCHAR);
      DebugMsg(
         ("CertSetCertificateContextProperty CERT_FRIENDLY_NAME_PROP_ID"));
      if (CertSetCertificateContextProperty(
             pCertContext, CERT_FRIENDLY_NAME_PROP_ID, 0, &cdblob)) {
         DebugMsg("Success");
      } else {
         // Error
         DebugMsg("Error 0x%x", GetLastError());
      ...
      if (CertSetCertificateContextProperty(
             pCertContext, CERT_DESCRIPTION_PROP_ID, 0, &cdblob)) {
         DebugMsg("Success");
      } else {
         // Error
         DebugMsg("Error 0x%x", GetLastError());
      ...
      hStore = CertOpenStore(
         ...
         0,
         0,
         MachineCert ? CERT_SYSTEM_STORE_LOCAL_MACHINE
      ...
      if (!hStore) {
         ...
         if (err == ERROR_ACCESS_DENIED)
            DebugMsg(
               "**** CertOpenStore failed with 'access denied' are  you running as administrator?");
         else
      ...
      if (!CertAddCertificateContextToStore(
             hStore, pCertContext, CERT_STORE_ADD_REPLACE_EXISTING, 0)) {
         // Error
         DebugMsg("Error 0x%x", GetLastError());
         return 0;
      } else {
