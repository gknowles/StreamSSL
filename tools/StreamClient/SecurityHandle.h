...
public:
   ...
   ~CredentialHandle() noexcept { Close(); }
   ...
   friend void
   attach(CredentialHandle & CredentialHandle, CredHandle value) noexcept {
   ...
   friend void
   swap(CredentialHandle & left, CredentialHandle & right) noexcept {
   ...
   friend bool operator==(
      CredentialHandle const & left,
      CredentialHandle const & right) noexcept {
      return get(left).dwLower == get(right).dwLower
         && get(left).dwUpper == get(right).dwUpper;
   }
...
struct ConstCertContextTraits : public HandleTraits<PCCERT_CONTEXT> {
   static void Close(Type value) { CertFreeCertificateContext(value); }
};
...
public:
   ...
   ~SecurityContextHandle() noexcept { Close(); }
   ...
   friend CtxtHandle
   get(SecurityContextHandle const & SecurityContextHandle) noexcept {
      return SecurityContextHandle.m_value;
   }

   friend CtxtHandle *
   set(SecurityContextHandle & SecurityContextHandle) noexcept {
   ...
   friend void attach(
      ...
      CtxtHandle value) noexcept {
      SecurityContextHandle.Close();
      SecurityContextHandle.m_value = value;
   }

   friend CtxtHandle
   detach(SecurityContextHandle & SecurityContextHandle) noexcept {
   ...
   friend void
   swap(SecurityContextHandle & left, SecurityContextHandle & right) noexcept {
   ...
   friend bool operator==(
      SecurityContextHandle const & left,
      SecurityContextHandle const & right) noexcept {
      return get(left).dwLower == get(right).dwLower
         && get(left).dwUpper == get(right).dwUpper;
   }
