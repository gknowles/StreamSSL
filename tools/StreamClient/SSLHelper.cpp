...
bool MatchCertHostName(PCCERT_CONTEXT pCertContext, LPCWSTR hostname) {
   ...
   auto dwCommonNameLength = CertGetNameString(
      pCertContext, CERT_NAME_ATTR_TYPE, 0, (void *)szOID_COMMON_NAME, 0, 0);
...
HRESULT FindCertificateByName(
   PCCERT_CONTEXT & pCertContext,
   const LPCTSTR pszSubjectName) {
   HCERTSTORE hMyCertStore = NULL;
   ...
   // If we ever really start using user names there's probably a better
   // scheme.
   //
   ...
   if (pszSubjectName) {
      ...
      if (pCertContext) {
         ...
         DebugMsg(
            "**** Error 0x%x returned by CertFindCertificateInStore", Err);
         return HRESULT_FROM_WIN32(Err);
      }
   } else
