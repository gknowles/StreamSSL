...
private:
   ...
   static const int MaxMsgSize =
      16000; // Arbitrary but less than 16384 limit, including MaxExtraSize
   static const int MaxExtraSize =
      50; // Also arbitrary, current header is 5 bytes, trailer 36
   CHAR writeBuffer[MaxMsgSize + MaxExtraSize];      // Enough for a whole
                                                     // encrypted message
   CHAR readBuffer[(MaxMsgSize + MaxExtraSize) * 2]; // Enough for two whole
...
public:
   ...
   std::function<bool(
      ...
      ServerCertAcceptable;
   std::function<SECURITY_STATUS(
      PCCERT_CONTEXT & pCertContext,
