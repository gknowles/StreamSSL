...
// The CSSLClient class, this declares an SSL client side implementation that
// requires
...
PSecurityFunctionTable CSSLClient::SSPI(void) {
   return g_pSSPI;
}
...
HRESULT CSSLClient::Initialize(
   LPCWSTR ServerName,
   const void * const lpBuf,
   const int Len) {
   HRESULT hr = S_OK;
   ...
   hr =
      CreateCredentialsFromCertificate(set(m_ClientCreds), get(hCertContext));
   if
      FAILED(hr) return hr;
   ...
   hr = g_pSSPI->QueryContextAttributes(
      &get(m_hContext), SECPKG_ATTR_STREAM_SIZES, &Sizes);
...
// Establish SSPI pointer and correct credentials (meaning pick a certificate)
// for the SSL server
...
// Because SSL is message oriented these calls send (or receive) a whole
// message
int CSSLClient::RecvPartial(LPVOID lpBuf, const ULONG Len) {
   if (plainTextBytes > 0) { // There are stored bytes, just return them
      DebugMsg("There is cached plaintext %d bytes", plainTextBytes);
      if (false)
         PrintHexDump(plainTextBytes, plainTextPtr);
      ...
      if (Len >= plainTextBytes) {
         ...
         DebugMsg(
            "All %d cached plaintext bytes can be returned", plainTextBytes);
         if (false)
            PrintHexDump(plainTextBytes, plainTextPtr);
         memcpy_s(lpBuf, Len, plainTextPtr, plainTextBytes);
         plainTextBytes = 0;
         return bytesReturned;
      } else { // More bytes are stored than the caller requested, so return
               // some, store the rest until the next call
         memcpy_s(lpBuf, Len, plainTextPtr, Len);
         plainTextPtr += Len;
         plainTextBytes -= Len;
         DebugMsg(
            "%d cached plaintext bytes can be returned, %d remain",
            Len,
            plainTextBytes);
         if (false)
            PrintHexDump(plainTextBytes, plainTextPtr);
   ...
   while (scRet == SEC_E_INCOMPLETE_MESSAGE) {
      int freeBytesAtStart =
         static_cast<int>((CHAR *)readPtr - &readBuffer[0]);
      int freeBytesAtEnd = static_cast<int>(sizeof(readBuffer))
         - readBufferBytes - freeBytesAtStart;
      if (freeBytesAtEnd == 0) // There is no space to add more at the end of
                               // the buffer
      {
         if (freeBytesAtStart > 0) // which ought to always be true at this
                                   // point
         {
            // Move down the existing data to make room for more at the end
            // of the buffer
            memmove_s(
               readBuffer,
               sizeof(readBuffer),
               readPtr,
               static_cast<int>(sizeof(readBuffer)) - freeBytesAtStart);
      ...
      err = m_SocketStream->RecvPartial(
         (CHAR *)readPtr + readBufferBytes, freeBytesAtEnd);
      ...
      if ((err == SOCKET_ERROR) || (err == 0)) {
         if (WSA_IO_PENDING == m_SocketStream->GetLastError())
            ...
            DebugMsg(
               "RecvMsg failed, the socket was closed by the other host");
         else
   ...
   // There's a legitimate case here of a server wanting to renegotiate the
   // session
   ...
   else { // More bytes were decoded than the caller requested, so return
      // some, store the rest until the next call
...
int CSSLClient::SendPartial(LPCVOID lpBuf, const ULONG Len) {
   ...
   err = m_SocketStream->SendMsg(
      writeBuffer,
      Buffers[0].cbBuffer + Buffers[1].cbBuffer + Buffers[2].cbBuffer);
   m_LastError = 0;

   DebugMsg(
      "SendPartial %d encrypted bytes to server",
      Buffers[0].cbBuffer + Buffers[1].cbBuffer + Buffers[2].cbBuffer);
   if (false)
      PrintHexDump(
         Buffers[0].cbBuffer + Buffers[1].cbBuffer + Buffers[2].cbBuffer,
...
// Negotiate a connection with the server, sending and receiving messages until
// the
...
SECURITY_STATUS CSSLClient::SSPINegotiateLoop(TCHAR * ServerName) {
   ...
   dwSSPIFlags = ISC_REQ_SEQUENCE_DETECT | ISC_REQ_REPLAY_DETECT
      ...
      | // We'll check the certificate ourselves
      ISC_REQ_STREAM;
   ...
   if (scRet != SEC_I_CONTINUE_NEEDED) {
      DebugMsg(
         "**** Error %#x returned by InitializeSecurityContext (1)", scRet);
      return scRet;
   }

   // Send response to server if there is one.
   if (OutBuffers[0].cbBuffer != 0 && OutBuffers[0].pvBuffer != NULL) {
      cbData = m_SocketStream->SendMsg(
         OutBuffers[0].pvBuffer, OutBuffers[0].cbBuffer);
      if (cbData != (int)OutBuffers[0].cbBuffer) {
         DebugMsg(
            "**** Error %d sending data to server (1)", WSAGetLastError());
   ...
   while (scRet == SEC_I_CONTINUE_NEEDED || scRet == SEC_E_INCOMPLETE_MESSAGE
      ...
      if (0 == cbIoBuffer || scRet == SEC_E_INCOMPLETE_MESSAGE) {
         if (fDoRead) {
            cbData = m_SocketStream->RecvPartial(
               readBuffer + cbIoBuffer, sizeof(readBuffer) - cbIoBuffer);
      ...
      if (
         ...
         // Get the server supplied certificate in order to decide whether
         // it is acceptable
         ...
         HRESULT hr = g_pSSPI->QueryContextAttributes(
            ...
            set(hServerCertContext));

         if (FAILED(hr)) {
            if (hr == SEC_E_INVALID_HANDLE)
               DebugMsg(
                  "QueryContextAttributes for cert returned SEC_E_INVALID_HANDLE, which is normal");
            else
               DebugMsg("Couldn't get server certificate, hr=%#x", hr);
         } else {
            DebugMsg("Server Certificate returned");
            ServerCertNameMatches = MatchCertHostName(
               get(hServerCertContext), ATL::CW2T(ServerName));
            hr = CertTrustedServer(get(hServerCertContext));
            ServerCertTrusted = hr == S_OK;
            bool IsServerCertAcceptable = ServerCertAcceptable == nullptr;
            if (!IsServerCertAcceptable)
               IsServerCertAcceptable = ServerCertAcceptable(
                  get(hServerCertContext),
                  ServerCertTrusted,
                  ServerCertNameMatches);
            if (!IsServerCertAcceptable)
               return SEC_E_UNKNOWN_CREDENTIALS;
         }

         if (OutBuffers[0].cbBuffer != 0 && OutBuffers[0].pvBuffer != NULL) {
            cbData = this->m_SocketStream->SendMsg(
               OutBuffers[0].pvBuffer, OutBuffers[0].cbBuffer);
      ...
      if (scRet == SEC_E_OK) {
         ...
         // application
         // protocol layer stuff. It needs to be saved. The application
         // layer
      ...
      if (FAILED(scRet)) {
         DebugMsg(
            "**** Error %#x returned by InitializeSecurityContext (2)", scRet);
         break;
      }
      ...
      if (scRet == SEC_I_INCOMPLETE_CREDENTIALS) {
         ...
         //
         // Note the a server will NOT send an issuer list if it has the
         // registry key
         ...
         if (scRet == SEC_E_OK) {
            ...
            DebugMsg(
               "**** Error %08x returned by GetNewClientCredentials", scRet);
            break;
         }
      }
...
SECURITY_STATUS CSSLClient::GetNewClientCredentials() {
   ...
   //
   // Note the a server will NOT send an issuer list if it has the registry
   // key
   // HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL
   // has a DWORD value called SendTrustedIssuerList set to 0
   //
   ...
   Status = g_pSSPI->QueryContextAttributes(
      &get(m_hContext), SECPKG_ATTR_ISSUER_LIST_EX, (PVOID)&IssuerListInfo);
   if (Status != SEC_E_OK) {
   ...
   DebugMsg(
      "Issuer list information returned, issuers = %d",
      IssuerListInfo.cIssuers);
   ...
   if (!hCertContext)
      DebugMsg(
         "No suitable client certificate is available to return to the server");
...
SECURITY_STATUS CSSLClient::CreateCredentialsFromCertificate(
   PCredHandle phCreds,
   ...
   if (Status != SEC_E_OK) {
      ...
      if (Status == SEC_E_UNKNOWN_CREDENTIALS)
         DebugMsg(
            "**** Error: 'Unknown Credentials' returned by AcquireCredentialsHandle. LastError=%d",
            dw);
      else
         DebugMsg(
