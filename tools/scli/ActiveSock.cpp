...
CActiveSock::CActiveSock(HANDLE StopEvent) {
   ...
   RecvInitiated = false;
   RecvTimeoutSeconds =
      1; // Default timeout is 1 seconds, encourages callers to set it
   SendTimeoutSeconds =
      1; // Default timeout is 1 seconds, encourages callers to set it
   ...
   int rc = true;
   setsockopt(
      ActualSocket, IPPROTO_TCP, TCP_NODELAY, (char *)&rc, sizeof(int));
}
...
bool CActiveSock::Connect(LPCTSTR HostName, USHORT PortNumber) {
   ...
   if (!bSuccess) {
      ...
      DebugMsg(
         "**** WsaConnectByName Error %d connecting to \"%S\" (%S)",
         LastError,
         HostName,
         PortName);
      closesocket(ActualSocket);
   ...
   iResult = setsockopt(
      ActualSocket, SOL_SOCKET, 0x7010 /*SO_UPDATE_CONNECT_CONTEXT*/, NULL, 0);
   if (iResult == SOCKET_ERROR) {
      LastError = WSAGetLastError();
      DebugMsg(
         "setsockopt for SO_UPDATE_CONNECT_CONTEXT failed with error: %d",
         LastError);
      closesocket(ActualSocket);
      return false;
   }
   //// At this point we have a connection, so set up keepalives so we can
   /// detect if the host disconnects
   //// This code is commented out because it does not seen to be helpful
   // BOOL so_keepalive = TRUE;
   // int iResult = setsockopt(ActualSocket, SOL_SOCKET, SO_KEEPALIVE, (const
   // char *)&so_keepalive, sizeof(so_keepalive));
   //	if (iResult == SOCKET_ERROR){
   //		LastError = WSAGetLastError();
...
// Receives up to Len bytes of data and returns the amount received - or
// SOCKET_ERROR if it times out
int CActiveSock::RecvPartial(LPVOID lpBuf, const ULONG Len) {
   WSABUF buffer;
   WSAEVENT hEvents[2] = {read_event, m_hStopEvent};
   DWORD
   bytes_read = 0, msg_flags = 0;
   int rc;

   if (RecvInitiated) {
      // Special case, the previous read timed out, so we are trying again,
      // maybe it completed in the meantime
      rc = SOCKET_ERROR;
      LastError = WSA_IO_PENDING;
      RecvEndTime = 0;
   } else {
      // Normal case, the last read completed normally, now we're reading
      // again
      ...
      rc = WSARecv(
         ...
         &msg_flags,
         &os,
         NULL); // Start an asynchronous read
      LastError = WSAGetLastError();
   }
   ...
   if (RecvEndTime == 0)
      RecvEndTime =
         CTime::GetCurrentTime() + CTimeSpan(0, 0, 0, RecvTimeoutSeconds);

   // Now wait for the I/O to complete if necessary, and see what happened
   bool IOCompleted = false;

   if ((rc == SOCKET_ERROR) && (LastError == WSA_IO_PENDING)) // Read in
                                                              // progress,
                                                              // normal case
   {
      CTimeSpan TimeLeft = RecvEndTime - CTime::GetCurrentTime();
      DWORD dwWait,
         milliSecondsLeft = (DWORD)TimeLeft.GetTotalSeconds() * 1000;
   ...
   if (IOCompleted) {
      ...
      if (
         WSAGetOverlappedResult(
            ActualSocket, &os, &bytes_read, true, &msg_flags)
         && (bytes_read > 0)) {
         LastError = 0;
         if (bytes_read == Len) // We got what was requested
            RecvEndTime = 0;    // Restart the timer on the next read
         return bytes_read;     // Normal case, we read some bytes, it's all
                                // good
      } else {                  // A bad thing happened
...
// Receives exactly Len bytes of data and returns the amount received - or
// SOCKET_ERROR if it times out
int CActiveSock::RecvMsg(LPVOID lpBuf, const ULONG Len) {
   ULONG bytes_received = 0, total_bytes_received = 0;

   RecvEndTime = 0; // Tell RecvPartial to restart the timer

   while (total_bytes_received < Len) {
      bytes_received = RecvPartial(
...
void CActiveSock::SetSendTimeoutSeconds(int NewSendTimeoutSeconds) {
   ...
   if (NewSendTimeoutSeconds > 0) {
      ...
      SendEndTime =
         CTime::GetCurrentTime() + CTimeSpan(0, 0, 0, SendTimeoutSeconds);
   }
}
...
int CActiveSock::SendPartial(LPCVOID lpBuf, const ULONG Len) {
   ...
   if ((rc == SOCKET_ERROR) && (LastError == WSA_IO_PENDING)) // Write in
                                                              // progress
   {
      ...
      dwWait = WaitForMultipleObjects(
         2, hEvents, false, (DWORD)TimeLeft.GetTotalSeconds() * 1000);
      if (dwWait == WAIT_OBJECT_0 + 1) // The write event
         IOCompleted = true;
   } else if (!rc) // if rc is zero, the write was completed immediately,
                   // which is common
      IOCompleted = true;
   ...
   if (IOCompleted) {
      ...
      if (WSAGetOverlappedResult(
             ActualSocket, &os, &bytes_sent, true, &msg_flags)) {
         if (bytes_sent == Len) // Everything that was requested was sent
            SendEndTime =
               0; // Invalidate the timer so it is set next time through
         return bytes_sent;
      }
   }
   return SOCKET_ERROR;
...
int CActiveSock::SendMsg(LPCVOID lpBuf, const ULONG Len) {
   ULONG bytes_sent = 0, total_bytes_sent = 0;

   SendEndTime = 0; // Invalidate the timer so SendPartial can reset it.

   while (total_bytes_sent < Len) {
      bytes_sent =
         SendPartial((char *)lpBuf + total_bytes_sent, Len - total_bytes_sent);
