...
CTransport::CTransport(SOCKET s, CListener * Listener)
   : // constructor requires a socket already assigned
   IsConnected(false)
   , m_Listener(Listener)
   , SSLServer(NULL) {
   ...
   if
      ...
      if (hr == SEC_E_INVALID_TOKEN)
         m_Listener->LogWarning(
            _T("SSL token invalid, perhaps the client rejected our certificate"));
      else if (hr == CRYPT_E_NOT_FOUND)
         m_Listener->LogWarning(
            _T("A usable SSL certificate could not be found"));
      else if (hr == E_ACCESSDENIED)
         m_Listener->LogWarning(
            _T("Could not access certificate store, is this program running with administrative privileges?"));
      else if (hr == SEC_E_UNKNOWN_CREDENTIALS)
         m_Listener->LogWarning(
            _T("Credentials unknown, is this program running with administrative privileges?"));
      else if (hr == SEC_E_CERT_UNKNOWN)
         m_Listener->LogWarning(
            _T("The returned client certificate was unacceptable"));
      else {
         CString s;
         s.Format(
            _T("SSL could not be used, hr =0x%lx, lasterror=0x%lx"), hr, err);
