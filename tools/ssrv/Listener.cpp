...
// CListerner object, listens for connections on one thread, and initiates a
// worker
...
// This is the individual worker process, all it does is start, change its name
// to something useful,
...
UINT __cdecl CListener::ListenerWorker(LPVOID v) {
   CListener * Listener =
      (CListener *)v; // See _beginthread call for parameter definition
...
CListener::ErrorType CListener::Initialize(int TCPSocket) {
   ...
   for (i = 0, AI = AddrInfo; AI != NULL; AI = AI->ai_next) {
      // Did we receive more addresses than we can handle?  Highly unlikely,
      // but check anyway.
      if (i == FD_SETSIZE)
         break;

      // Only support PF_INET and PF_INET6.  If something else, skip to next
      // address.
      if ((AI->ai_family != AF_INET) && (AI->ai_family != AF_INET6))
         continue;

      // StringCchPrintf(MsgText, _countof(MsgText), _T("::OnInit i = %d,
      // ai_family = %d"), i, AI->ai_family);
      // LogWarning(MsgText);
      ...
      // StringCchPrintf(MsgText, _countof(MsgText), _T("::OnInit Created
      // m_hSocketEvents[%d], handle=%d"), i, m_hSocketEvents[i]);
      // LogWarning(MsgText);

      m_iListenSockets[i] = WSASocket(
         AI->ai_family, SOCK_STREAM, 0, NULL, 0, WSA_FLAG_OVERLAPPED);
      if (m_iListenSockets[i] == INVALID_SOCKET)
         return SocketUnusable;

      // StringCchPrintf(MsgText, _countof(MsgText), _T("::OnInit binding
      // m_iListenSockets[%d] to sa_family=%u sa_data=%s len=%d"), i,
      // AI->ai_addr->sa_family, AI->ai_addr->sa_data, AI->ai_addrlen);
      // LogWarning(MsgText);
   ...
   // StringCchPrintf(MsgText, _countof(MsgText), _T("::OnInit no errors,
   // m_iNumListenSockets = %d"), m_iNumListenSockets);
   // LogWarning(MsgText);

   return NoError;
}

// Start listening for connections, if a timeout is specified keep listening
// until then
void CListener::BeginListening(
   std::function<void(ISocketStream * StreamSock)> actualwork) {
   m_actualwork = actualwork;
   m_ListenerThread = AfxBeginThread(ListenerWorker, this);
}

// Stop listening, tells the listener thread it can stop, then waits for it to
// terminate
void CListener::EndListening(void) {
   m_StopEvent.SetEvent();
   if (m_ListenerThread) {
      WaitForSingleObject(
         m_ListenerThread->m_hThread, INFINITE); // Will auto delete
   }
   m_ListenerThread = NULL;
}
...
void CListener::Listen(void) {
   ...
   // StringCchPrintf(MsgText, _countof(MsgText), _T("CListener::Listen
   // m_iNumListenSockets= %d"), m_iNumListenSockets);
   // LogWarning(MsgText);

   hEvents[0] = m_StopEvent;
   // StringCchPrintf(MsgText, _countof(MsgText), _T("CListener::Listen
   // hEvents[0] = m_StopEvent = %d"), m_StopEvent);
   // LogWarning(MsgText);

   // Add the events for each socket type (two at most, one for IPv4, one for
   // IPv6)
   for (int i = 0; i < m_iNumListenSockets; i++) {
      hEvents[i + 1] = m_hSocketEvents[i];
      // StringCchPrintf(MsgText, _countof(MsgText), _T("CListener::Listen
      // hEvents[%d] = m_hSocketEvents[%d] = %d"), i+1, i,
      // m_hSocketEvents[i]);
      // LogWarning(MsgText);
   }
   ...
   while (true) {
      // StringCchPrintf(MsgText, _countof(MsgText), _T("CListener::Listen
      // entering WaitForMultipleObjects for %d objects"),
      // m_iNumListenSockets+1);
      // LogWarning(MsgText);

      dwWait = WaitForMultipleObjects(
         m_iNumListenSockets + 1, hEvents, false, INFINITE);
      ...
      // StringCchPrintf(MsgText, _countof(MsgText), _T("CListener::Listen
      // event %d triggered, iMyIndex = %d"), dwWait, iMyIndex);
      // LogWarning(MsgText);
      ...
      // A request to open a socket has been received, begin a thread to
      // handle that connection
      DebugMsg("Starting worker");

      CTransport * Transport =
         new CTransport(iReadSocket, this); // Deleted by worker thread
   ...
   while (m_WorkerThreadCount) {
      ...
      Sleep(1000);
      DebugMsg(
         "Waiting for all workers to terminate: worker thread count = %i",
         m_WorkerThreadCount);
      m_WorkerThreadLock.Lock();
   };
