...
// Cached server credentials (a handle to a certificate), usually these do not
// change
// because the server name does not change, but occasionally they may change
// due to SNI
CredHandle CSSLServer::g_ServerCreds = {0};
CString CSSLServer::g_ServerName = CString();

// The CSSLServer class, this declares an SSL server side implementation that
// requires
...
CSSLServer::CSSLServer(CPassiveSock * SocketStream)
   ...
   , m_LastError(0) {
   m_hContext.dwLower = m_hContext.dwUpper =
      ((ULONG_PTR)((INT_PTR)-1)); // Invalidate it
}
...
// Avoid using (or exporting) g_pSSPI directly to give us some flexibility in
// case we want
// to change implementation later
PSecurityFunctionTable CSSLServer::SSPI(void) {
   return g_pSSPI;
}

// Return an ISocketStream interface to the SSL connection to anyone that needs
// one
ISocketStream * CSSLServer::getSocketStream(void) {
   return m_SocketStream; // for now, return 'this' later, once we can do SSL
}

// Set up the connection, including SSL handshake, certificate
...
HRESULT CSSLServer::Initialize(const void * const lpBuf, const int Len) {
   ...
   if (lpBuf && (Len > 0)) { // preload the IO buffer with whatever we already
                             // read
      readBufferBytes = Len;
   ...
   if (!SSPINegotiateLoop()) {
      ...
      if (IsUserAdmin())
         ...
         std::cout
            << "SSL handshake failed, perhaps because you are not running as administrator."
            << std::endl;
      int le = GetLastError();
...
int CSSLServer::Recv(void * const lpBuf, const int Len) {
   ...
   //
   // Initialize security buffer structs, basically, these point to places to
   // put encrypted data,
   // for SSL there's a header, some encrypted data, then a trailer. All three
   // get put in the same buffer
   // (ReadBuffer) and then decrypted. So one SecBuffer points to the header,
   // one to the data, and one to the trailer.
   ...
   while (scRet == SEC_E_INCOMPLETE_MESSAGE) {
      err = m_SocketStream->Recv(
         (CHAR *)readPtr + readBufferBytes,
         (int)(sizeof(readBuffer) - readBufferBytes - ((CHAR *)readPtr - &readBuffer[0])));
   ...
   // Locate the data buffer because the decrypted data is placed there. It's
   // almost certainly
   // the second buffer (index 1) and we start there, but search all but the
   // first just in case...
   ...
   else { // More bytes were decoded than the caller requested, so return an
      // error
      m_LastError = WSAEMSGSIZE;
      return SOCKET_ERROR;
   }

   // See if there was any extra data read beyond what was needed for the
   // message we are handling
   // TCP can sometime merge multiple messages into a single one, if there is,
   // it will amost
   // certainly be in the fourth buffer (index 3), but search all but the
   // first, just in case.
   ...
   if (pExtraDataBuffer) { // More data was read than is needed, this happens
                           // sometimes with TCP
      DebugMsg(" ");
      DebugMsg(
         "Some extra ciphertext was read (%d bytes)",
...
int CSSLServer::Send(const void * const lpBuf, const int Len) {
   ...
   // all positioned in the right place to be sent across the TCP connection
   // as one message.
   //
   ...
   err = m_SocketStream->Send(
      writeBuffer,
      Buffers[0].cbBuffer + Buffers[1].cbBuffer + Buffers[2].cbBuffer);
   m_LastError = 0;

   DebugMsg(
      "Send %d encrypted bytes to client",
      Buffers[0].cbBuffer + Buffers[1].cbBuffer + Buffers[2].cbBuffer);
   PrintHexDump(
      Buffers[0].cbBuffer + Buffers[1].cbBuffer + Buffers[2].cbBuffer,
      writeBuffer);
...
// Negotiate a connection with the client, sending and receiving messages until
// the negotiation succeeds or fails
bool CSSLServer::SSPINegotiateLoop(void) {
   ...
   bool ContextHandleValid =
      (m_hContext.dwUpper != ((ULONG_PTR)((INT_PTR)-1)));
   ...
   if (ClientCertAcceptable) // If the caller wants a client certificate,
                             // request one
   {
   ...
   DebugMsg(
      "Started SSPINegotiateLoop with %d bytes already received from client.",
      readBufferBytes);

   scRet = SEC_E_INCOMPLETE_MESSAGE;

   // Main loop, keep going around this until the handshake is completed or
   // fails
   while (scRet == SEC_I_CONTINUE_NEEDED || scRet == SEC_E_INCOMPLETE_MESSAGE
      ...
      if (readBufferBytes == 0 || scRet == SEC_E_INCOMPLETE_MESSAGE) {
         // Read some more bytes if available, we may read more than is needed
         // for this phase of handshake
         err = m_SocketStream->Recv(
            readBuffer + readBufferBytes,
            sizeof(readBuffer) - readBufferBytes);
         m_LastError = 0;
         if (err == SOCKET_ERROR || err == 0) {
            if (WSA_IO_PENDING == m_SocketStream->GetLastError())
               DebugMsg("Recv timed out");
            ...
            if (err == readBufferBytes)
               ...
               DebugMsg(
                  "Received %d handshake bytes from client, total is now %d ",
                  err,
                  readBufferBytes);
            CSSLHelper SSLHelper((const byte *)readBuffer, readBufferBytes);
            SSLHelper.TraceHandshake();
            if (SSLHelper.IsClientInitialize()) {
               // Figure out what certificate we might want to use, either
               // using SNI or the local host name
               CString serverName = SSLHelper.GetSNI();
               if (
                  (!g_ServerCreds.dwLower
                   && !g_ServerCreds.dwUpper) // No certificate handle stored
                  || (serverName.Compare(g_ServerName)
                      != 0)) // Requested names are different
               {             //
                  if ((g_ServerCreds.dwLower
                       || g_ServerCreds.dwUpper)) // Certificate handle stored
                     g_pSSPI->FreeCredentialsHandle(&g_ServerCreds);
                  if (serverName.IsEmpty()) // There was no hostname
                                            // supplied by SNI
                  ...
                  if (SelectServerCert) {
                     status =
                        SelectServerCert(pCertContext, (LPCTSTR)serverName);
                     if (FAILED(status)) {
                        DebugMsg(
                           "SelectServerCert returned an error = 0x%08x",
                           status);
                        scRet = SEC_E_INTERNAL_ERROR;
                        break;
                     }
                  } else
                     status = CertFindServerByName(
                        pCertContext,
                        (LPCTSTR)serverName); // Add "true" to look in
                                              // user store, "false",
                                              // or nothing looks in
                                              // machine store
                  g_ServerName = (SUCCEEDED(status)) ? serverName : CString();
                  if (SUCCEEDED(status))
                     status = CreateCredentialsFromCertificate(
                        &g_ServerCreds, pCertContext);
                  if (FAILED(status)) {
                     DebugMsg(
                        "Failed handling server initialization, error = 0x%08x",
                        status);
                     scRet = SEC_E_INTERNAL_ERROR;
                     break;
                  }
      ...
      //
      // InBuffers[1] is used for storing extra data that SSPI/SCHANNEL
      // doesn't process on this run around the loop.
      //
      ...
      scRet = g_pSSPI->AcceptSecurityContext(
         ...
         ContextHandleValid ? &m_hContext
                            : NULL, // The context handle if we have one,
                                    // ask to make one if this is first call
         &InBuffer,                 // Input buffer list
         dwSSPIFlags,               // What we require of the connection
         0,                         // Data representation, not used
         ContextHandleValid ? NULL : &m_hContext, // If we don't yet have a
                                                  // context handle, it is
                                                  // returned here
         &OutBuffer, // [out] The output buffer, for messages to be sent to
                     // the other end
         &dwSSPIOutFlags, // [out] The flags associated with the negotiated
                          // connection
      ...
      if (
         scRet == SEC_E_OK || scRet == SEC_I_CONTINUE_NEEDED
         || (FAILED(scRet)
             && (0 != (dwSSPIOutFlags & ASC_RET_EXTENDED_ERROR)))) {
         if (OutBuffers[0].cbBuffer != 0 && OutBuffers[0].pvBuffer != NULL) {
            // Send response to client if there is one
            err = m_SocketStream->CPassiveSock::Send(
               OutBuffers[0].pvBuffer, OutBuffers[0].cbBuffer);
            m_LastError = 0;
            if (err == SOCKET_ERROR || err == 0) {
               DebugMsg(
                  "Send handshake to client failed: %d",
                  m_SocketStream->GetLastError());
      ...
      // At this point, we've read and checked a message (giving scRet) and
      // maybe sent a response (giving err)
      // as far as the client is concerned, the SSL handshake may be done,
      // but we still have checks to make.

      if (scRet == SEC_E_OK) { // The termination case, the handshake worked
                               // and is completed, this could as easily be
                               // outside the loop

         // Ensure a client certificate is checked if one was requested, if
         // none was provided we'd already have failed
         if (ClientCertAcceptable) {
            PCERT_CONTEXT pCertContext = NULL;
            HRESULT hr = g_pSSPI->QueryContextAttributes(
               &m_hContext, SECPKG_ATTR_REMOTE_CERT_CONTEXT, &pCertContext);
            ...
            if (FAILED(hr))
               ...
               if (false && debug && pCertContext)
                  ShowCertInfo(
                     pCertContext, _T("Server Received Client Certificate"));
               // All looking good, now see if there's a client
               // certificate, and if it is valid
               bool acceptable = ClientCertAcceptable(
                  pCertContext, S_OK == CertTrustedClient(pCertContext));
         ...
         // Now deal with the possibility that there were some data bytes
         // tacked on to the end of the handshake
         if (InBuffers[1].BufferType == SECBUFFER_EXTRA) {
            readPtr = readBuffer + (readBufferBytes - InBuffers[1].cbBuffer);
            readBufferBytes = InBuffers[1].cbBuffer;
            DebugMsg(
               "Handshake worked, but received %d extra bytes",
               readBufferBytes);
         } else {
            readBufferBytes = 0;
            readPtr = readBuffer;
         ...
         m_LastError = scRet;
         return false;
      } else { // We won't be appending to the message data already in the
               // buffer, so store a reference to any extra data in case it
               // is useful
         if (InBuffers[1].BufferType == SECBUFFER_EXTRA) {
            readPtr = readBuffer + (readBufferBytes - InBuffers[1].cbBuffer);
            readBufferBytes = InBuffers[1].cbBuffer;
            DebugMsg(
               "Handshake working so far but received %d extra bytes we can't handle",
               readBufferBytes);
            m_LastError = WSASYSCALLFAILURE;
            return false;
         } else {
...
HRESULT CSSLServer::Disconnect(void) {
   ...
   Status = g_pSSPI->AcceptSecurityContext(
      &g_ServerCreds, // Which certificate to use, already established
      &m_hContext,    // The context handle if we have one, ask to make one if
                      // this is first call
      NULL,           // Input buffer list
      dwSSPIFlags,    // What we require of the connection
      0,              // Data representation, not used
      NULL, // Returned context handle, not used, because we already have one
      &OutBuffer, // [out] The output buffer, for messages to be sent to the
                  // other end
      &dwSSPIOutFlags, // [out] The flags associated with the negotiated
                       // connection
      &tsExpiry);      // [out] Receives context expiration time
...
// Create credentials (a handle to a certificate) by selecting an appropriate
// certificate
// We take a best guess at a certificate to be used as the SSL certificate for
// this server
SECURITY_STATUS CSSLServer::CreateCredentialsFromCertificate(
   PCredHandle phCreds,
   ...
   Status = g_pSSPI->AcquireCredentialsHandle(
      ...
      (LPWSTR)UNISP_NAME,  // Name of package
   ...
   if (Status != SEC_E_OK) {
      ...
      if (Status == SEC_E_UNKNOWN_CREDENTIALS)
         DebugMsg(
            "**** Error: 'Unknown Credentials' returned by AcquireCredentialsHandle. Be sure app has administrator rights. LastError=%d",
            dw);
      else
         DebugMsg(
            "**** Error 0x%x returned by AcquireCredentialsHandle. LastError=%d.",
            Status,
            dw);
      return Status;
   }
