...
SelectServerCert(PCCERT_CONTEXT & pCertContext, LPCTSTR pszSubjectName) {
   SECURITY_STATUS status = CertFindServerByName(
      pCertContext, pszSubjectName); // Add "true" to look in user store,
                                     // "false", or nothing looks in machine
                                     // store
   if (pCertContext)
      wcout << "Server certificate requested for " << pszSubjectName
            << ", found \"" << (LPCWSTR)GetCertName(pCertContext) << "\""
            << endl;
   return status;
}
...
bool ClientCertAcceptable(PCCERT_CONTEXT pCertContext, const bool trusted) {
   if (trusted)
      ...
      cout << "An untrusted";
   wcout << " client certificate was returned for \""
         << (LPCWSTR)GetCertName(pCertContext) << "\"" << endl;
   return NULL != pCertContext; // Meaning any certificate is fine, trusted or
                                // not, but there must be one
}

int _tmain(int argc, TCHAR * argv[], TCHAR * envp[]) {
   if (!IsUserAdmin())
      cout << "WARNING: The server is not running as an administrator."
           << endl;
   ...
   Listener->BeginListening([](ISocketStream * const StreamSock) {
      ...
      CString s;
      char MsgText[100]; // Because the simple text messages we exchange are
                         // char not wchar

      cout << "A connection has been made, worker started, sending hello"
           << endl;
